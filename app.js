...
app.use(expressCors({
	allowedOrigins: ['http://localhost:*','http://*.xdiscovery.com:*']
}))
...
app.get('/api/v1/node/search/', function (req,res) {
...
app.get('/api/v1/node/:identifier', function (req,res) {
	search_manager.get_node(parseInt(req.params.identifier),function(data) {
		res.json(data)
	})
})

app.post('/api/v1/node/:identifier/user', function (req,res) {
...
app.post('/api/v1/node/:identifier/neighbors', function (req,res) {
